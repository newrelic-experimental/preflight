...
export class CopilotSdkAdapter implements PlatformAdapter {
  ...
  getHookInstallInstructions(): string {
    return [
      ...
      '   configures all of the below automatically, including the hooks file,',
      '   MCP registration, and (optionally) the SDK usage extension in step 4.',
      ...
      '     }',
      '   }',
      '   The NEW_RELIC_AI_PLATFORM=copilot prefix is required — the hooks-runner',
      '   does not inherit env vars set on the MCP server registration in step 3',
      '   below, so without it every event falls through to the claude-code',
      '   platform default.',
