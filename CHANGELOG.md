...
- `ToolCallRecord.agentType` is now backfilled from the same parent Agent-tool correlation (`agentTypeByAgentId`: `spawnedAgentId` + `subagentType`) that cost-by-type already uses, once `agentId` is known. Audit-trail and security events that read `record.agentType` receive the type when that signal exists; the field stays undefined otherwise. Type is per spawned agent, not per tool-use.
