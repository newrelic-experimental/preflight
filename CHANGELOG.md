...
- **A session's "Session Quality" card showed "Diff Apply NaN%" and "Test Pass NaN%" on nearly every session.** The session detail response carried that session's raw signal counts under the same key the dashboard reads the two rates from, and those counts have no rate fields. A session with no diff or test signals now hides the card entirely, matching what the other session-detail paths already did.
