...
export class KiroAdapter implements PlatformAdapter {
  ...
  getHookInstallInstructions(): string {
    return [
      ...
      '     }',
      '   }',
      '   NEW_RELIC_AI_PLATFORM is required, not optional: Kiro sets no',
      '   ambient env var this server can detect itself by, so without it',
      '   platform detection falls through to generic-mcp and every',
      '   normalized-name metric (unique_files_read, bash_commands_run, etc.)',
      ...
      '    ]',
      '  }',
      '  MCP_CLIENT=kiro is required on the hook command itself — this',
      '  subprocess is spawned separately from the MCP server above and never',
      '  inherits its env, so it needs its own explicit platform stamp.',
    ].join('\n');
  }
