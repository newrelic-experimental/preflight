...
- **Bundled pricing overlay for Copilot-visible models missing from the vendored pricing table** (e.g. `claude-opus-5`, several `gpt-5.x`/`grok`/`kimi` variants), which previously resolved to `$0` cost. Applied automatically at startup unless a custom pricing file is configured — see `copilot-pricing/README.md` for the gap-fill contract and known limitations.
